...
| Nạp dữ liệu         | POST   | /api/flashsale/warmup     | (Dành cho Admin) Bơm dữ liệu từ MySQL lên Redis để chuẩn bị.| Không     |

---

## Nhóm 4: Cổng Thanh Toán VNPay (Mã hóa SHA512)
*Đường dẫn gốc: `/api/payment`*

| Chức năng           | Method | Endpoint                  | Chức năng chi tiết                                          | Cần Token |
|---------------------|:------:|---------------------------|-------------------------------------------------------------|:---------:|
| Tạo Link Giao Dịch  | POST   | /api/payment/create_url   | Hash thông tin và trả về URL chuyển sang Web của VNPay.     | Có        |
| Bắt Callback (IPN)  | GET    | /api/payment/vnpay_ipn    | Webhook chạy ngầm. Ngân hàng báo trạng thái đơn về Backend. | Không     |
